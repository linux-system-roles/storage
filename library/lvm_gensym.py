...
version_added: "2.4"
description:
    - "Module generates unique, default names for a volume group
      and logical volume"
...
def name_is_unique(name, used_names):
    """Check if name is contained in the used_names list
...
def get_unique_name_from_base(base_name, used_names):
    """Generate a unique name given a base name and a list of used names,
       and return that unique name
    """
    counter = 0
...
def get_vg_name(host_name, lvm_facts):
    """Generate a base volume group name, verify its uniqueness,
       and return that unique name
    """
...
def get_lv_name(fs_type, mount_point, lvm_facts):
    """Return a unique logical volume name based on specified
       file system type, mount point, and system facts
    """
...
def run_module():
    ...
    host_name = facts.ansible_facts(module)['nodename'].lower()
    host_name = host_name.replace('.', '_').replace('-', '_')

    result['lv_name'] = get_lv_name(module.params['fs_type'],
                                    module.params['mount'],
                                    lvm_facts)
    ...
    if result['lv_name'] != '' and result['vg_name'] != '':
        ...
        module.fail_json(msg="Unable to initialize both group "
                             "and volume names")
