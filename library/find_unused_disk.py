...
description:
    ...
    - "Disks that meet all conditions are considered 'empty' and returned
      (using kernel device name) in a list."
        - "1. No known signatures exist on the disk, with the exception
          of partition tables."
        - "2. If there is a partition table on the disk, it contains
          no partitions."
        - "3. The disk has no holders to eliminate the possibility of it being
          a multipath or dmraid member device."
        - "4. Device can be opened with exclusive access to make sure no other
          software is using it."
    - "If no disks meet all criteria, "Unable to find unused disk"
      will be returned."
    - "Number of returned disks defaults to first 10,
      but can be specified with I(max_return) argument."
author: Eda Zhou (@edamamez)
options:
    max_return:
        description:
            - "Sets the maximum number of unused disks to return."
        default: 10
        type: int
requirements:
    - "blkid"
...
disk_name:
    description:
        - "Information about unused disks"
    returned: success
    type: complex
    contains:
        disks:
            description:
                - "Unused disk(s) that have been found"
            returned: success
            type: list
...
def no_signature(run_command, blkid_bin, disk_path):
    """Return true if no known signatures exist on the disk."""
    signatures = run_command([blkid_bin, '-p', disk_path])
...
def run_module():
    ...
    for disk in ansible_facts['devices'].keys():
        ...
        if (no_partitions and
                no_signature(run_command, blkid_bin, '/dev/' + disk) and
                no_holders(disk) and
                can_open('/dev/' + disk)):
